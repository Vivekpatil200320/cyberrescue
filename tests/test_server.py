...
class TestDockerCallWithRetry:
    ...
    def test_returns_result_on_first_success(self):
        ...
        from cyberrescue import server

        result = asyncio.run(server._docker_call_with_retry(lambda: "ok"))
        assert result == "ok"
    ...
    def test_retries_transient_failure_then_succeeds(self, monkeypatch):
        ...
        monkeypatch.setattr(server, "RETRY_BASE_DELAY_S", 0)
        calls = {"n": 0}

        def flaky():
            calls["n"] += 1
            if calls["n"] < 3:
                raise self._docker_exc()
            return "recovered"

        result = asyncio.run(server._docker_call_with_retry(flaky))
    ...
    def test_raises_after_exhausting_attempts(self, monkeypatch):
        ...
        with pytest.raises(DockerException):
            asyncio.run(server._docker_call_with_retry(always_fails))
        assert calls["n"] == server.RETRY_ATTEMPTS
