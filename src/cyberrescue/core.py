"""Tool logic shared by the stdio MCP server (server.py) and the HTTP backend.

Kept free of any FastMCP/@mcp.tool() decoration so it can be imported by
both the local Claude Desktop entrypoint and the web backend without
pulling MCP-specific machinery into the web deployment.
"""
